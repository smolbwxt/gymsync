...
5. **Google Calendar sync (sub-phase, gated on OAuth consent approval ~1-2 weeks — start the Google Cloud verification at Phase H kickoff):** `connected_accounts` + `session_calendar_syncs` + Edge Function sync per spec Flow 10.
   - **Schema staged 2026-07-18 (Task 5), client/Edge Function work still gated on the OAuth consent screen:** migration `20260724000002_connected_accounts_calendar_sync.sql` — both tables live, RLS enabled, owner/organizer read-only, zero client write surface on either table (server-side/Edge-Function-only, enforced by RLS + a table-wide REVOKE re-granted only on a non-secret column list). Vault check: `supabase_vault` is installed and used for one global secret (`push_drain_auth`, push-cron), but there's no repo precedent for per-row per-user Vault secrets — building one now with no OAuth callback to exercise it would be untested, so `access_token`/`refresh_token` are plain `text` columns, honestly plaintext-at-rest today (no client can read them — column-level REVOKE + re-GRANT, proven in pgTAP, see `connected_accounts_calendar_sync_test.sql`). Real Vault-based encryption is deferred to whoever builds the Edge Function, before the first real token is written. NO OAuth client code, NO Edge Function, NO token-exchange logic exists yet — still blocked on the user's Google Cloud OAuth consent screen approval.
