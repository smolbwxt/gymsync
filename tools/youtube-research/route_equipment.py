"""Targeted router: programming AROUND an equipment constraint.

Owner 2026-08-26. The consult asks what the athlete has to lift with, and
as of today that answer actually reaches the generator — but the generator
only FILTERS the catalog with it. Nothing tells it how to program when the
barbell is missing: which movement patterns still get covered, what a
dumbbell-only lower body looks like, how to load a hinge with no bar, when
bands and bodyweight are genuinely equivalent and when they are not.
...
Usage:  python route_equipment.py [--top 25]
"""
import argparse
import glob
import json
...
TRIGGERS = re.compile(
    r"\b(home gym|garage gym|minimal(ist)? (equipment|setup|gym)|"
    r"dumbbell.only|dumbbells? only|no (barbell|bench|rack|gym|equipment)|"
    r"without (a )?(barbell|bench|rack|gym|weights)|"
    r"bodyweight (only|training|workout|program)|band(s|ed)? (only|training)|"
    r"resistance bands?|kettlebell (only|program|training)|"
    r"hotel (gym|room)|travel(ing)? (workout|training)|"
    r"limited equipment|equipment (you )?(need|don.?t need)|"
    r"machine.only|cables? only|budget (gym|home gym)|"
    r"what to buy|home gym essentials)\b", re.I)
CONTEXT = re.compile(
    r"\b(sets?|reps?|program|train(ing)?|exercise|substitut|alternativ|"
    r"replace|swap|hypertrophy|strength|progress(ion|ive)?|overload|"
    r"pattern|squat|hinge|press|row|pull)\b", re.I)
WINDOW = 180
