"""Targeted router: accessory/isolation work FOR STRENGTH athletes.
...
Usage:  python route_strength_accessories.py [--top 16]
"""
import argparse
import glob
import json
...
TRIGGERS = re.compile(
    r"\b(accessor(?:y|ies)|assistance (?:work|exercise|lift|movement)s?|"
    r"weak.?point|lagging muscle|isolation (?:work|exercise|lift)s?|"
    r"tricep\w* (?:for|and) (?:bench|lockout|press)|"
    r"hypertrophy (?:for|block|phase).{0,20}strength|"
    r"bigger muscle.{0,20}strong|build muscle to (?:lift|get strong)|"
    r"supplemental (?:lift|work|exercise))", re.I)
CONTEXT = re.compile(
...
def main() -> None:
    ...
    os.makedirs("passes/strength-accessories", exist_ok=True)
    picks = []
    ...
    for i in range(0, len(picks), 4):
        ...
        json.dump(out, open(f"passes/strength-accessories/b-{i // 4 + 1:02d}.json",
                            "w", encoding="utf-8"))
    print(f"wrote {(len(picks) + 3) // 4} batches -> passes/strength-accessories/")
