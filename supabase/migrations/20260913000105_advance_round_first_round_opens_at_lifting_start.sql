...
--
-- Fix-forward from Stream D review finding 2
-- (.superpowers/sdd/2026-09-13-group-session-phase-b-plan/review-data.md):
-- D3's round-close predicate (20260913000102_session_round_engine.sql)
-- fell back to '-infinity' when round_started_at was NULL, reasoning "the
-- first round opened when the session did." That conflates
-- sessions.started_at (schedule/join time) with lifting_started_at (set
-- when the crew actually starts lifting, after any warm-up phase --
-- 20260803000004_session_warmup_phase.sql). set_logs_reject_prelive_session
-- (20260803000002_set_logs_reject_prelive.sql:16) gates only on session
-- state, not on lifting_started_at, so nothing DB-side stops a set from
-- being logged during warm-up; under the old fallback that set would have
-- counted toward closing round 1.
--
-- CREATE OR REPLACE public.advance_round, one change: the close
-- predicate's lower bound becomes COALESCE(v_round_started_at,
-- v_lifting_started_at, '-infinity'::timestamptz), reading
-- lifting_started_at in the same SELECT ... FOR UPDATE that already reads
-- state/round/round_started_at. The function body below is otherwise
-- byte-identical to 20260913000102's definition -- its inline commentary
-- included, unedited here even where the COALESCE line it explains has
-- moved on; that cleanup is separate from this fix. REVOKE/GRANT are
-- untouched: CREATE OR REPLACE preserves the existing ACL for an
-- unchanged signature. The COMMENT ON FUNCTION gains one appended
-- sentence.
