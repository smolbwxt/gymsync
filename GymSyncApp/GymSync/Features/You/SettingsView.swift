...
/// Redesign Phase 1 (four-tab reorientation, 2026-08): everything the You
/// TAB held before the widget-grid rebuild — account details (profile row,
/// stat tiles), the Settings group box, QA tools, Delete Account, Sign Out
/// and the Build-number footer — moved here WHOLESALE, functionality
/// unchanged. `YouTabView` is now the widget grid; its SETTINGS widget
/// pushes this screen. The `navigationDestination`s declared below resolve
/// against YouTabView's enclosing `NavigationStack` (this view no longer
/// owns one).
struct SettingsView: View {
    ...
    var body: some View {
            ...
            .background(theme.bg)
            // Pushed destination (not a tab root anymore) — system bar with
            // its back button, inline title. The tab-root "in-content title
            // + hidden bar" idiom no longer applies here.
