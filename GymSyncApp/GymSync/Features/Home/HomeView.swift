...
struct HomeView: View {
    ...
    ///
    /// I1: repointed at A4's `distinctTrainingDays` — B1's own one-function
    /// placeholder copy of this file (which named the function
    /// `daysThisWeek`) lost the integration collision to A's full module,
    /// per that file's own header comment. Same body, same signature shape,
    /// only the name and the parameter label changed.
    private var daysThisWeek: Int {
        WeeklyGoalProgressMath.distinctTrainingDays(sessions: historySessions)
    }
