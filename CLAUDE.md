...
- **Proof runs get their own ref (2026-09-18):** `ios.yml` cancels in-progress runs per branch off master, so an implementer's push kills a render the controller is waiting on (45 minutes lost on the PR-celebration pick card). A run whose ARTIFACT is the deliverable is dispatched (`gh workflow run ios.yml --ref …`) on a throwaway `proof/<what>-<sha>` branch at a commit whose build-test is already green; delete the branch once the card is cut.
- **Gate ordering (2026-09-18):** `backend.yml` runs pgTAP against the LIVE database, so a drop applied live reddens every branch lacking the matching test retirement, master included. Merge the retirement PR in the same window as the gate, before forking new data branches. This applies to STORAGE deletes too — grep `supabase/tests` for the object before any live delete.
- **Token economy II (2026-09-19, owner: "blowing through my weekly usage"):** the spend is turns × transcript size, so (1) NEVER resume an agent above ~250k tokens — fix rounds from file:line findings and scoped re-reviews go to a FRESH Sonnet with a `-U3` package, "warm cache" is not a reason; (2) cap legs at ~250k / two tasks per Opus leg, retire at every push; (3) the CONTROLLER is the most expensive reader: one background `gh run watch <id> --exit-status` per run (one wake-up) instead of 30-minute expiring monitors, no message on a non-event, updates ≤ 5 lines unless a decision is needed; (4) a NEW SESSION at every PR/phase boundary — ledger + memory + plan are the handoff; (5) small late commits get a type self-check before push (optionals into non-optional params, `let x = default` fixture fields, every caller of a changed signature incl. tests) — three CI-only compile slips cost three re-runs in Phase C1; (6) a final whole-branch Opus review is for phases, not for small PRs.
